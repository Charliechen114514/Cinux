...
析构函数的描述揭示了一个比较精妙的设计决策：它只释放用户空间（PML4[0..255]）的页表子树，而不碰内核空间的条目。原因很简单——内核空间的页表页不属于任何单个 `AddressSpace`，它们是启动时建立的全局资源，只有用户空间页表才是这个实例"私有"的。递归释放走的是 PDPT -> PD -> PT 三级，到 PT 级就停下了——PT 条目指向的是数据页，数据页的生命周期由进程自己的内存分配器管理，不属于 `AddressSpace` 的管辖范围。
