...
```
    ...
    ├── err & 0x01 == 0 (P=0, 页不存在)
    │   ├── 页对齐 CR2
    │   ├── PMM::alloc_page() 分配物理页
    │   ├── VMM::map() 建立映射 (FLAG_PRESENT | FLAG_WRITABLE)
    │   ├── return → IRETQ 恢复执行
    │   └── 分配/映射失败 → 落入 fatal 路径
    ├── err & 0x01 == 1 (P=1, 权限违反 / 保留位)
    │   └── → 打印诊断并 panic
    ▼
...
这几行代码做了两件事。第一，`fault_addr & ~0xFFFULL` 把故障地址向下对齐到页边界——CR2 里的地址可能是页内的任意偏移，但我们的映射操作是以整页为单位的，所以必须先对齐。`~0xFFFULL` 这个掩码清掉了低 12 位，等价于 `fault_addr & ~(PAGE_SIZE - 1)`。

第二，调用 PMM 的 `alloc_page()` 分配一个物理页，然后调用 VMM 的 `map()` 建立映射，标志位设为 Present + Writable。映射成功后直接 `return`，CPU 执行 IRETQ 回到触发异常的那条指令重新执行，此时页表已经建好了，访问正常通过。

如果 PMM 分配失败（内存耗尽，`phys == 0`）或映射失败（`map` 返回 false），代码直接落入下方的 fatal 路径——打印诊断信息并 panic。
...
1. **写时复制（Copy-on-Write）**：当前 #PF handler 只处理 P=0 的情况，P=1 的权限违反直接 panic。未来可以扩展 handler，在检测到 P=1、W/R=1 的组合时识别为 CoW fault。研究 `fork()` 时如何将子进程的页表指向父进程的物理页并标记为只读，写时再分配新页。
...
- OSDev Wiki: [Page Fault](https://wiki.osdev.org/Exceptions#Page_Fault) — #PF 异常的错误码位域详解，P=0（not-present）和 P=1（protection violation）的区别。
