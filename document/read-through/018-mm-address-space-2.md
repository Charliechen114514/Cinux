...
关键在于递归终止条件：`if (level > LEVEL_PT)` 才继续递归。当 `level == LEVEL_PT` 时不再递归——因为 PT 条目指向数据页，不属于页表基础设施。`AddressSpace` 只管理页表结构（PML4/PDPT/PD/PT），数据页的生命周期由上层分配器负责。递归顺序是"先深入，后释放"——保证释放父级页表页之前子级已处理完毕。
...
### Decision: free_subtree 在 PT 层停止而非继续释放数据页

**问题**：地址空间销毁时，用户空间的映射指向的数据页应该如何处理？

**本项目的做法**：只释放页表结构页（PDPT/PD/PT），不释放数据页。数据页的生命周期由上层管理。

**备选方案**：递归到底，把 PT 条目指向的数据页也归还给 PMM。

**为什么不选备选方案**：数据页可能被多个地址空间共享（如未来的共享内存）或被内核引用。AddressSpace 不知道引用计数，贸然释放会导致 use-after-free。当前的"只释放页表结构"策略是最安全的保守选择。
