...
│ ctors        │    │   RSP -= 8 (ABI)   │    │ linker.ld         │
│ kernel_main  │    │   jump_to_usermode │    │   VMA=0x400000    │
└──────────────┘    └────────────────────┘    ├───────────────────┤
                                              │ CMakeLists.txt    │
                                              │   ELF→bin→embed.o │
                                              └───────────────────┘
...
USER_STACK_TOP、USER_ABI_RSP_OFFSET 和 USER_STACK_PAGES 定义在 usermode.hpp 头文件中，供内核其他模块使用。static_assert 在编译期验证栈对齐：0x7FFFFF000 - 8 = 0x7FFFFEFF8，对 16 取模等于 8，满足 x86-64 SysV ABI 的要求。
...
usermode_init 非常简洁——调用 usermode_init_asm（配置 STAR/EFER MSR），然后分配 per-CPU GS 数据页并写入 IA32_KERNEL_GS_BASE MSR。GS 数据页的三个槽位：gs:0 存内核栈指针（初始为 0，后续由调度器更新），gs:8 存用户 RSP 暂存区，gs:16 存返回值暂存区。
...
GS 数据页在 usermode_init 中分配——此时 PMM 已经初始化完毕。gs:0 初始化为 0，后续由调度器在 context_switch 时通过 PerCPU::update_syscall_stack() 更新为当前任务的内核栈指针。gs:16 作为返回值暂存区也在这里就绪。虚拟地址写入 IA32_KERNEL_GS_BASE MSR，这样 syscall_entry 执行 swapgs 后就能通过 gs:0、gs:8、gs:16 访问这些数据。
...
_syscall1 和 _syscall3 是用户态的单参数和三参数 syscall 封装。内联汇编把 syscall 号放入 RAX（"a"约束），参数分别放入 RDI（"D"）、RSI（"S"）、RDX（"d"）。clobber 列表包含 rcx（SYSCALL 自动覆盖为 RIP）、r11（SYSCALL 自动覆盖为 RFLAGS）、memory（syscall 可能修改内存）。基于这些封装 sys_write、sys_read 等。sys_exit 使用 _syscall1 且在之后标记 `__builtin_unreachable()`——因为 sys_exit 不返回。sys_yield 同样使用 _syscall1。
