...
头文件还声明了三个公开接口：syscall_init() 配置 MSR 和初始化分发表，syscall_register() 注册 handler，syscall_get_kernel_rsp() 读回保存的内核栈指针。以及两个 extern "C" 声明：syscall_entry()（汇编入口，不能被 C++ 编译器 name mangling）和 syscall_dispatch()（被汇编调用，同样需要 C 链接）。
...
调用号参考 Linux x86-64 约定。使用 enum class 而不是宏或常数，这样编译器会做类型检查——你不能意外地把一个随机整数当系统调用号传。分发表大小 256 是一个合理的上限，Linux 早期也用类似的大小。
...
init 函数的第一步是保存内核栈指针——这个值会在 GS 数据页初始化时写入 gs:0，被 syscall_entry 加载。然后清空分发表，所有槽位设为 nullptr。
...
三个 MSR 的配置。STAR 的 [47:32] 和 [63:48] 都设为 0x08（内核代码段选择子）——SYSCALL 方向用 [47:32]，SYSRET 方向用 [63:48]。LSTAR 写入 syscall_entry 的地址。SFMASK 设为 0x200（bit 9 = IF），这样每次 syscall 进入时中断自动关闭，防止在栈切换的脆弱窗口被打断。
...
dispatch 逻辑非常直接：边界检查、空指针检查、调用 handler。这个函数被 syscall.S 中的 call syscall_dispatch 调用，所以必须是 extern "C" 链接。两个返回 -1 的路径分别对应无效调用号和未注册的 handler。

### sys_write.cpp -- 带地址验证的输出
...
sys_write 是三个 handler 中最值得细看的。首先验证 buf_virt < 0x800000000000——这个常量是 x86-64 canonical address 的分界线，低于此值的是用户空间地址。如果用户传入一个内核地址（比如 0xFFFFFFFF80000000），这个检查会直接拦截。然后检查 fd==1（stdout），目前只支持标准输出。通过验证后，把用户虚拟地址强转为 char 指针逐字节输出到串口。

说句实话，这里的地址验证是最基本的——它只检查地址范围，不检查该地址是否真的被映射到当前进程的地址空间。更完善的实现需要配合页表遍历或 copy_from_user 机制。但对于教学 OS 来说，这个简单的检查足以说明安全验证的重要性。
...
sys_yield 就是 yield 的 syscall 封装——没什么额外逻辑。调用 yield 后调度器会选择下一个任务执行，当前任务回到就绪队列。
