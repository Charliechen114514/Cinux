...
sys_read 的签名与所有 syscall handler 一致——6 个 uint64_t 参数（第一个是 syscall 号，后面 5 个是实际参数），返回 int64_t。多出的参数位用匿名 uint64_t 占位。fd=0 表示 stdin，buf_virt 是用户空间的缓冲区地址，count 是最大读取字节数。
