...
所有函数用 extern "C" 包裹，匹配标准 libc 签名。这样做有两个好处：C++ 代码可以正常调用（因为头文件声明在 extern "C" 里，链接器会找 C 链接名的符号），而且如果以后有纯 C 的内核代码也能链接到这些函数。__restrict__ 是 GCC 扩展关键字，告诉编译器 dest 和 src 指向的内存不重叠，允许更激进的优化。
...
add 的流程很直观：找空位、算长度、检查合法性、拷贝路径、设置指针、标记占用。注意 memcpy 的第三个参数是 len + 1——包含 null 终止符。如果 8 个槽位都满了，返回 false。
...
### g_global_fd_table 和 __cxa_guard
