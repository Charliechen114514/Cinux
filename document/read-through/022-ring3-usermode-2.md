...
本文精讲 tag 022 的核心文件——usermode.S（汇编）和 usermode.cpp（C++）。usermode.S 包含两个汇编函数：`usermode_init_asm` 配置 STAR/EFER/SFMASK 三个 MSR，`jump_to_usermode` 执行 SYSRET 从 Ring 0 切换到 Ring 3。usermode.cpp 提供两个 C++ 函数：`usermode_init()` 调用汇编初始化并打印日志，`launch_first_user()` 构建完整的用户地址空间并调用 jump_to_usermode。
...
`0x400000` 是 x86-64 ELF 的标准加载地址（4MB），Linux 用户程序的默认 text 段基址也是这个值。`0x7FFFFF000` 接近 2GB 边界但留了一页的 guard page。这些地址都在用户空间（低于 0x800000000000 canonical 边界），Ring 3 可以访问。

头文件还声明了两个 public 函数和 extern "C" 的 jump_to_usermode 汇编入口：
...
extern "C" 块中的 jump_to_usermode 使用 C 链接避免 name mangling，参数通过 RDI/RSI/RDX 传入（System V AMD64 ABI）。
...
```asm
movq $0x08, %rdx           # 内核 CS 选择子值
shlq $16, %rdx             # 移到 EDX[31:16] -> STAR[63:48]
orq  $0x08, %rdx           # EDX[15:0] -> STAR[47:32]
xorq %rax, %rax            # EAX = 0 (低 32 位未使用)
movq $0xC0000081, %rcx     # MSR_STAR
wrmsr                       # EDX:EAX -> MSR
```

这里 `shlq $16` 把 0x08 移到 EDX 的 bit 31-16，所以 EDX = 0x00080008。wrmsr 把 EDX:EAX 写入 STAR MSR，结果 STAR = 0x00080008_00000000。STAR[63:48] = 0x08（SYSRET 内核 CS 基），STAR[47:32] = 0x08（SYSCALL 内核 CS 基）。SYSRET 由此计算 CS = 0x08+16|3 = 0x1B，SS = (0x08+8)|3 = 0x13。

注意这里必须用 `shlq $16` 而不是 `shlq $32`。`wrmsr` 只读取 EDX（RDX 低 32 位）和 EAX（RAX 低 32 位），RDX 高 32 位完全被忽略。如果用 `shlq $32`，0x08 被移到了 RDX 的 bit 39-32，但 EDX（bit 31-0）仍然是 0，所以 STAR[63:48] = 0，SYSRET 计算出 CS = 0+16|3 = 0x13（数据段选择子 + RPL3），CPU 尝试在数据段上取指——Triple Fault 或 #GP。
...
SFMASK 值 0x200 表示 SYSCALL 时自动清除 RFLAGS 的 IF 位（关中断）。这只影响 SYSCALL 方向，SYSRET 从 R11 恢复 RFLAGS，不受 SFMASK 影响。
...
前三行设置 SYSRET 的三个关键寄存器：RCX 是目标 RIP（用户入口地址），RSP 是用户栈顶，RDI 是传给用户程序的参数。R11 通过 push/pop 设置为 0x202——bit 9 是 IF（中断使能），bit 1 是 RFLAGS 的保留位（必须为 1）。SYSRET 后 RFLAGS = R11 & 0x3C7FD7 | 2，所以 IF 会被保留，用户态下中断是开启的。
...
`sysretq` 是关键指令——`q` 后缀表示 64 位操作数大小（REX.W 前缀），确保返回 64 位模式而非 32 位兼容模式。执行后 CPU 的 CPL 变为 3，RIP = RCX = 0x400000，RFLAGS = R11 & mask | 2，CS = 0x1B，SS = 0x13。

### usermode.cpp — launch_first_user()

这是整个 Ring 3 启动的 C++ 编排函数，按 9 个步骤构建用户环境。

用户代码机器码定义在匿名命名空间中：

```cpp
constexpr uint8_t kUserCode[] = {
    0xFA,           // cli  (特权指令, Ring 3 触发 #GP)
    0xF4,           // hlt  (特权指令)
    0xEB, 0xFC,     // jmp -4 (跳回 cli)
};
```

4 个字节的机器码：cli 清中断（Ring 3 不允许），hlt 停机（Ring 3 也不允许），jmp -4 无限循环。这段代码的唯一目的就是触发 #GP 来证明特权隔离有效。
...
FLAG_USER (bit 2) 是关键——没有这个位，Ring 3 无法访问该页。结合 FLAG_PRESENT 和 FLAG_WRITABLE，这些页对 Ring 3 是可读可写的。

地址空间构建的核心流程：创建 AddressSpace，分配代码页并映射到 0x400000，通过高半区直接映射写入机器码字节，分配并映射 4 页栈空间，复制 identity mapping，激活地址空间，设置 TSS.RSP0，调用 jump_to_usermode。
