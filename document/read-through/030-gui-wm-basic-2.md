...
构造函数接收标题、位置和尺寸，自动分配唯一 ID。虚函数 on_key、on_paint、is_terminal 提供子类扩展点。is_terminal 这个看似奇怪的虚函数是为了避免 unsafe static_cast——在 tick 回调里需要判断窗口是否是 Terminal 以调用 poll_output，用虚函数比 RTTI 安全得多。

Window 是 non-copyable 的，因为每个 Window 独占一个 Canvas（Canvas 持有 back_buf_ 堆内存）。如果允许拷贝，两个 Window 指向同一个 back_buf_ 会在析构时 double-free。
...
draw_content 清空内容区为浅灰色，然后调用虚函数 on_paint 给子类机会画自定义内容。基类的 on_paint 是空操作。
...
WindowManager 是一个 Meyers' singleton，通过 instance() 静态方法获取。它管理最多 64 个窗口指针和 16 个桌面图标。常量 DESKTOP_COLOR 定义了深青色桌面背景。
...
add_window 与 create 类似，但接收外部预分配的 Window 指针。这用于 Terminal 等需要子类化的窗口——外部先 `new Terminal(...)`，然后通过 add_window 注册到窗口管理器。
...
composite 的流程是：clear → draw_desktop_icons → blit all windows → draw_cursor → flip。桌面图标画在所有窗口之前，所以它们总是在最底层。

handle_mouse 实现了拖拽状态机。MouseDown 做 hit test → 关闭按钮检测 → raise → 标题栏检测 → 开始拖拽。MouseMove 在拖拽状态下移动窗口。MouseUp 结束拖拽。

handle_key 就一行——如果焦点窗口存在，调 on_key。简单但有效的事件路由。
