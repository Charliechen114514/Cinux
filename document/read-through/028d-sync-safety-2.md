...
这里全部使用 `memory_order_relaxed`——tick_count_ 和 current_slice_ 是独立的计数器，不需要与其他操作建立 happens-before 关系。我们只需要保证 fetch_add 本身是原子的（不会出现半写的状态），relaxed 就足够了。
...
- Intel SDM: Vol.3A Section 8.1 — 原子操作与 LOCK 前缀
