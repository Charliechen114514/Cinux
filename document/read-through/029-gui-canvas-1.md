...
开头的 delete 检查处理了重复初始化的情况——如果 Canvas 被多次 init，先释放旧的 back buffer。然后从 Framebuffer 获取维度信息，计算 stride（每行的 uint32_t 元素数）= pitch / 4，分配 back buffer 并清零为黑色。

这里有一个很容易忽略的细节：分配大小是 `stride * height_` 而不是 `width_ * height_`。在大多数 QEMU 配置中 pitch 等于 width*4，所以两者结果相同，但在某些硬件配置中 pitch 可能大于 width*4。用 stride 保证所有写入都在合法范围内。

### Canvas::init(uint32_t w, uint32_t h)

离屏 Canvas 的初始化——不需要硬件 framebuffer：
...
void Canvas::init(uint32_t w, uint32_t h) {
    ...
    width_     = w;
    height_    = h;
    pitch_     = w * 4;  // 无硬件对齐，pitch = width * 4
...
和上一个 init 的区别：front_buf_ 设为 nullptr（flip 是空操作），pitch 直接等于 w*4（没有硬件对齐约束），分配大小用 width*height 就够了（因为没有 padding）。这种模式用于窗口系统的离屏渲染——每个窗口有自己的 Canvas，画完后通过 blit 合成到屏幕 Canvas。
...
- OSDev Wiki: [Drawing In a Linear Framebuffer](https://wiki.osdev.org/Drawing_In_a_Linear_Framebuffer) — pixel addressing, fill rect optimization
- OSDev Wiki: [VESA BIOS Extensions](https://wiki.osdev.org/VESA_BIOS_Extensions) — linear framebuffer, pitch
- Linux: [DRM Internals](https://www.kernel.org/doc/html/v5.3/gpu/drm-internals.html) — modern framebuffer management
- SerenityOS: LibGfx Bitmap/Painter — analogous 2D canvas abstraction
