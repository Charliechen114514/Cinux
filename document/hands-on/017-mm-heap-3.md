...
本章的代码覆盖 `kernel/mm/heap.cpp` 的后半部分（约 180-353 行），包括 `free()`、`coalesce()`、`expand()` 和 `dump_stats()`。运行环境和前一章完全一致——PMM 和 VMM 必须已初始化，堆已通过 init 建立了初始空闲块。
...
关于 free 的错误处理策略，有两种选择：一是"fail-safe"（打印错误并返回，不修改堆状态），二是"fail-fatal"（直接 panic 或 halt）。Cinux 选择了前者——内核可能在其他方面还能继续工作，打印错误信息让开发者知道问题出在哪里。如果你希望更严格的错误处理（一旦检测到内存错误就停机），可以在 free 中把 `return` 改为 `__asm__ volatile("cli; hlt")`。
...
为什么不用排序链表？FreeRTOS 的 heap_4 把空闲链表按地址排序，合并时只需要检查前后邻居，一遍就够。Cinux 选择了无序链表（头插法释放），所以需要完整遍历来找邻居。这个取舍是有意为之的——排序链表在释放时需要找到正确的插入位置（O(n)），把搜索开销从 coalesce 转移到了 free，总开销不变但代码复杂度增加了。对于内核堆的规模（几十个块），两种方案的性能差异可以忽略不计。
...
两个方向的合并都要加上 HEADER_SIZE。这是因为两个相邻块之间有一个 header——合并后这个 header 不再需要，它的 32 字节空间变成了可用 payload。所以合并后的大小是 `survivor->size + HEADER_SIZE + absorbed->size`，而不是简单的 `survivor->size + absorbed->size`。忘记加 HEADER_SIZE 会导致合并后的块比实际小 32 字节，后续的分配可能踩到这块"丢失"的空间。
...
一个常见的 bug 是在摘除链表头节点时忘记更新 free_list_。由于头插法的特性，刚释放的块通常就是链表头节点。但在传递性合并的场景中（合并 A+B 后再合并 (A+B)+C），被吞并的块可能不在链表头部——这取决于释放的顺序和合并的方向。
...
expand 的核心假设是堆区域在虚拟地址空间中是连续的——新页面总是映射在 `base_ + size_` 的位置（堆的当前末尾）。这意味着虚拟地址从 `base_` 到 `base_ + size_ + expand_size` 是一段连续的已映射区域。如果 VMM 的 `map` 函数在这个地址范围映射失败（比如 PML4 表项还没分配），expand 会部分成功——一些页面被映射了，另一些没有，堆的总大小可能没有被更新。这种部分成功的状态是一个潜在的 bug 来源，但在实践中极少发生。
...
一个值得思考的边界情况：如果 alloc 在搜索空闲链表时发现一个块 magic 不正确（corrupt block），它会打印错误并返回 nullptr，而不是尝试扩展。这是一个有意的设计决策——如果堆已经被踩了，继续分配只会让情况更糟。正确的做法是停下来报告错误，让开发者排查。

### dump_stats 不仅仅是调试工具

虽然 dump_stats 名义上是一个调试函数，但它在验证堆分配器正确性方面扮演了核心角色。每次 alloc/free 循环后调用 dump_stats 可以快速检查 used_ 是否回到 0（无泄漏）、free_list 的大小是否合理（碎片化程度）、以及块的数量是否异常增长（coalesce 不工作）。

dump_stats 遍历的是空闲链表，不是堆中的所有块。已分配的块不在链表中，所以"已使用量"是从 used_ 成员变量读取的。如果你发现 free_total + used 和 total 对不上，可能是 coalesce 或 expand 的逻辑有问题，导致某些空间既没有被计入 used_ 也没有出现在空闲链表中。
...
这六步的顺序很重要。nullptr 检查必须在 header_from_ptr 之前——否则 nullptr 减去 32 会变成地址 0xFFFFFFFFFFFFFFE0，访问这个地址会触发 page fault。magic 校验必须在 free 标志检查之前——因为如果 magic 不对，free 标志的值也不可信（它可能被损坏覆写了）。更新 used_ 必须在设置 free=1 之前——虽然顺序对结果没有影响，但逻辑上"先更新统计再改状态"更清晰。
...
"相邻"不是指在空闲链表中紧挨着，而是在虚拟地址空间中紧挨着——块 X 的尾部（X 的地址 + HEADER_SIZE + X 的 size）恰好等于块 Y 的地址。这意味着 X 和 Y 在内存中是无缝连接的，中间没有任何间隙。
...
注意合并方向一中"从链表摘除 block"的搜索操作——这里不能用内层循环的 prev 指针，因为 prev 指向的是 curr 的前驱，不是 block 的前驱。所以需要一个额外的线性搜索来找到 block 的前驱节点。这是单链表删除操作的固有代价。

方向二（block 吞并 curr）的摘除更简洁，因为内层循环的 prev 恰好就是 curr 的前驱。这是两种合并方向实现复杂度不对称的原因。

合并后 block 指针的更新只发生在方向一——方向二不需要，因为 block 的起始地址不变，只是变大了。方向一中 `block = curr` 是必须的——否则下一轮循环还会用旧的 block 地址去检查相邻性，而那个地址的内容已经被合并到 curr 中了。
...
你也可以尝试不同的释放顺序来测试 coalesce 的不同路径。释放 A -> C -> B：先释放 A，没有相邻空闲块；再释放 C，也没有；最后释放 B，B 和 A 相邻（合并为 A+B），然后 A+B 和 C 相邻（合并为 A+B+C）。这种"中间块最后释放"的场景测试了 coalesce 的双向合并能力。
...
如果 expand 输出了 `[HEAP] OOM during expansion at offset ...`，说明物理内存不足以扩展堆。在 QEMU 默认配置（128 MB）下这不应该发生，除非 PMM 有 bug 或者其他子系统消耗了过多的物理页。你可以用 PMM 的 `free_page_count` 来检查当前可用的物理页数。
...
堆分配器现在是一个功能完备的动态内存管理器。但它还是一个"孤岛"——内核的 C++ 代码要用 new 和 delete，不会直接调用 g_heap.alloc() 和 g_heap.free()。下一章我们将把 operator new/delete 重定向到 Heap，并搭建双层测试体系来验证分配器的正确性。
...
**free 后分配同样大小失败**: 如果你释放了一个 64 字节的块，但再次分配 64 字节时失败，说明释放的块没有回到空闲链表。在 free 中打印 `block->next = free_list_` 前后的 free_list_ 值，确认链表头确实被更新了。

**expand 后 size_ 没有增长**: 如果 expand 映射了新页面但 `size_` 没有更新，下一次 expand 会把新页面映射到和上一次相同的位置（`base_ + size_` 还是旧值），覆盖之前的扩展。确保 `size_ += expand_size` 在 expand 的最后一步执行——在创建空闲块和打印消息之后。
...
**合并后 used_ 不归零**: 如果释放了所有分配的块但 used_ 不是 0，最可能的原因是 free 中 `used_ -= HEADER_SIZE + block->size` 的公式写错了——比如忘了加 HEADER_SIZE。在每次 free 后打印 used_ 的值，找到它开始偏离预期的那个 free 调用，然后检查那个块的大小是否正确。
...
**堆收缩（heap shrink）**: 当空闲链表中有大量尾部空闲空间时，释放末尾的物理页回 PMM。这需要确保被释放的页上没有已分配的块——实现难度比扩展大得多，因为需要扫描整个堆区域来确认。Linux 内核在内存紧张时会做类似的操作。

**coalesce 性能优化**: 当前的 O(n^2) 最坏复杂度在空闲块很多时可能成为瓶颈。改为维护按地址排序的链表，合并时只需一次 O(n) 遍历。或者改用 buddy 系统的合并策略——每个块记录它属于哪个 order（大小级别），释放时只和它的 buddy 合并，O(1) 时间复杂度。

**在 expand 中合并末尾空闲块**: 创建新的空闲块之前检查堆末尾是否已经有一个空闲块。如果是，直接扩展它而不是创建新块。这能减少碎片并避免 coalesce 的延迟。

**线程安全**: 当前的 free + coalesce 不是线程安全的——如果中断处理程序和主循环同时调用 free，链表会被踩。可以在 Heap 类中加入自旋锁保护。

本章完成的是堆分配器的核心功能——释放、合并和扩展。至此 init、alloc、free、coalesce、expand 五个函数全部就位。从下一章开始，我们将把 C++ 的 new/delete 接到堆分配器上，并通过双层测试体系（host + QEMU）验证整个实现的正确性。构建命令不变：`cmake --build build && ./build/run_qemu.sh`。
