...
这是一个我们在 035 踩过的真实坑，提前在这里预警。fork() 函数需要知道"当前函数的返回地址在哪里"，以便在子进程的栈上正确设置 ctx.rsp。在 debug 模式下，编译器会为每个函数生成标准的帧指针（push rbp; mov rbp, rsp），此时 [rbp+8] 就是返回地址。但 Release 模式（-O2）默认开启 -fomit-frame-pointer，RBP 变成通用寄存器，[rbp+8] 不再是返回地址。

如果你的 fork() 需要读取帧指针来定位栈帧，必须在函数上加 `__attribute__((optimize("no-omit-frame-pointer")))` 和 `noinline`，强制这个函数保留帧指针。
...
**实现约束**: 第一步通过 pid_alloc.alloc() 分配 PID。第二步用 aligned new 分配 Task 结构体。第三步 memcpy 整个父 TCB 到子 TCB。第四步修复子进程特有字段：新 TID、新 PID、ppid=父PID、state=Ready、parent=父、children=nullptr、exit_status=0。第五步分配 STACK_PAGES(4) 个物理页作为内核栈，映射到内核虚拟地址空间（保留一个 guard page 不映射）。第六步复制父进程栈内容到子进程栈——需要先通过 RBP 定位 fork 的返回地址在父栈上的位置，然后在子栈上做对应的重定位。第七步设置子进程的 ctx.rsp 和 ctx.rbp。第八步如果有地址空间就做 CoW 页表复制。第九步链入父进程的 children 列表。第十步加入调度器。

**踩坑预警**: 第六步和第七步是最容易出问题的地方。你必须确保子进程的 ctx.rsp 指向 fork 返回地址在子栈上的正确位置，而 ctx.rbp 指向调用者（fork 的调用者）的帧指针。如果 RBP 不是帧指针（Release 模式），整个计算就是垃圾。函数声明必须加 no-omit-frame-pointer 属性。另外栈复制要用父栈的"已使用量"（kernel_stack_top - 当前 RSP）来决定复制多少字节，而不是复制整个栈。

**验证**: 在 fork 返回后打印日志："fork: created child pid=X tid=Y parent_pid=Z"。在 QEMU 中应该看到这条日志，然后子进程被调度器调度后开始执行（此时它还不会走子进程路径——那是 trampoline 的事）。
...
1. **子进程 Double Fault + RSP 在用户空间**: 几乎可以确定是帧指针问题。检查 fork() 函数是否有 no-omit-frame-pointer 属性。在异常帧中 RSP 是一个很小的用户空间地址（如 0x7FFFF947F008），这就是 RBP 不是帧指针导致 ctx.rsp 计算为垃圾值。
...
| fork() | memcpy TCB + 新栈 + CoW 页表 | 必须保留帧指针 |
