...
**设计思路**: 每个 wrapper 遵循相同的模式——把系统调用号放入 rax，参数放入 rdi/rsi/rdx/r10/r8/r9，执行 syscall 指令，返回 rax 的值。sys_open 的参数是路径指针和 flags；sys_close 只有一个 fd 参数；sys_getdents 参数是 fd、缓冲区指针和缓冲区大小。
