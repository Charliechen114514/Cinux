...
本章分为三篇，本篇（第一篇）聚焦于 PIC 驱动的设计与实现。第二篇覆盖 PIT 定时器驱动，第三篇完成 IRQ stub 注册、路由表和 kernel_main 的集成。
...
注意 `PicICW` 命名空间中不仅定义了本章会用到的常量（`ICW1_INIT`、`ICW1_ICW4`、`ICW4_8086`），还定义了一些备选功能的位域常量——`ICW1_SINGLE = 0x02`（单芯片模式，我们不用，因为有两片 PIC）、`ICW4_AUTO_EOI = 0x02`（自动 EOI 模式，我们不选，原因在后面设计决策中讨论）。列出这些备选常量的目的是让头文件成为自文档化的参考——读者不需要去查 8259A datasheet 就能知道这个芯片还支持什么功能。
...
关于 IMR 的保存和恢复，这里有一个常见的问题：为什么不直接在 init() 末尾调用 `disable_all()` 把所有 IRQ 屏蔽掉，而要费劲保存和恢复之前的值？原因有两方面。第一，调用方可能在 init() 之前就已经通过 BIOS 设置了特定的屏蔽位（比如 BIOS 可能已经 unmask 了 IRQ0 和 IRQ1），直接 `disable_all()` 会破坏这些设置。第二，保存恢复的语义更清晰——init() 做的事情是"重新初始化 PIC 并保持屏蔽状态不变"，而不是"重新初始化 PIC 并屏蔽所有 IRQ"。后者是一种策略决策，不应该隐藏在 init() 里。

**验证**：编译通过即可。可以用 `cmake --build build -j$(nproc)` 验证。

---

## 深入理解：为什么 PIC 初始化必须重映射

很多教程在讲 PIC 初始化时会直接给出代码，但对"为什么要重映射"这个问题往往一笔带过。让我们详细拆解一下。

Intel 在设计 x86 保护模式时，把 INT 0x00 到 INT 0x1F（0-31）保留给了 CPU 异常。这些异常由 CPU 内部条件触发——比如除零（#DE, INT 0）、缺页（#PF, INT 14）、一般保护错误（#GP, INT 13）。当这些异常发生时，CPU 会自动将错误码压栈并跳转到对应的 IDT 条目。

IBM 在设计 PC/AT 时选择了 8259A 作为中断控制器，并把 Master PIC 的向量基址设为 0x08。这意味着 IRQ0（定时器）被映射到 INT 0x08，而 INT 0x08 正好是 #DF（Double Fault）。如果不去重映射，定时器中断每次到来，CPU 都会以为发生了 Double Fault——而你的 #DF handler 很可能只是打印一条错误信息然后 halt，于是内核就卡死了。

更微妙的是 IRQ13（协处理器错误）和 INT 0x0D（#GP）的重叠——如果 FPU 产生了一个错误中断，CPU 会跳到 #GP handler，handler 看到的是一个莫名其妙的"一般保护错误"，完全无法诊断。

重映射到 0x20-0x2F 的选择不是随意的。Intel 保留了 0x00-0x1F 给 CPU 异常（实际上只用了 0x00-0x1E，0x1F 是保留的），从 0x20（32）开始就是用户可用的向量号区域。Master PIC 基址 0x20 覆盖 IRQ0-7（INT 0x20-0x27），Slave PIC 基址 0x28 覆盖 IRQ8-15（INT 0x28-0x2F），16 个向量号正好连成一片，中间没有间隙。这个布局已经成为 x86 OS 开发的标准约定。

---

## 本篇小结

本篇介绍了 8259A PIC 的硬件架构、BIOS 默认映射与 CPU 异常向量的冲突问题、ICW1-ICW4 四步初始化序列的含义、以及 EOI 机制的原理。动手部分完成了 PIC 驱动的头文件和实现——`PIC::init()` 完成 IRQ 重映射，`PIC::send_eoi()` 处理中断结束签收，`PIC::mask()`/`unmask()` 控制单条 IRQ 线的开关。

下一篇我们将在此基础上构建 PIT 定时器驱动，配置 Channel 0 为 100 Hz 方波模式，编写 IRQ0 handler 来统计 tick 和打印 uptime。

## 参考资料

**Intel 手册**：
- Intel 64 and IA-32 Architectures SDM, Volume 3A, Section 6.2 -- Exception and Interrupt Vectors
