...
这一篇涉及的新增代码量比第一篇大得多——scheduler.cpp 新增了约 100 行代码，scheduler.hpp 新增了约 15 行声明。核心新增函数有六个：tick()、schedule()、idle_entry()、block()、unblock() 和 remove_task()。同时需要修改 init()、yield()、exit_current() 和 run_first() 来适配新的抢占式逻辑。
...
对比 Linux 的做法：Linux 的 CFS（Completely Fair Scheduler）不用固定时间片，而是给每个任务分配一个"虚拟运行时间"的比例，时间片根据任务数量和权重动态计算。一个 CPU 密集型任务在系统负载低时可能得到 6ms 的时间片，在负载高时可能只有 1-2ms。这种自适应方案远比固定时间片高效，但实现复杂度也高得多。Cinux 选择固定时间片是因为简单——教学内核不需要处理数千个并发任务。
...
对比 Linux 的做法：`scheduler_tick()` 只负责更新运行时统计和设置 `TIF_NEED_RESCHED` 标志，实际的上下文切换推迟到下一个 preemption point。这种延迟抢占模型更灵活，但需要维护 preempt_count 等复杂机制。Cinux 选择直接在 tick 中调用 schedule 是因为当前只有内核线程，没有用户态和抢占计数的概念。
...
为什么不用一个简单的 "halt 循环" 代替 idle 任务？技术上可以在 schedule() 中检测 "没有可运行任务" 时直接进入 `while(1) { hlt; }`，不需要创建专门的 idle 任务。但这种做法有几个问题：第一，进入 halt 循环后，调度器没有一个"当前任务"可以管理——g_per_cpu.current 指向谁？第二，如果需要统计 CPU 空闲时间、检测系统负载，idle 任务提供了一个挂载点——`if (current() == idle_task_)` 就能判断当前是否空闲。第三，idle 任务保持了一个统一的执行流模型——所有执行流都是 Task，不存在"调度器自己也在执行"的特殊情况。
...
另外要注意 `current_slice_` 的重置时机——它必须在 `schedule()` 被调用之前重置为 0（在 tick() 内部完成），而不是在 schedule() 内部重置。如果在 schedule() 内部重置，那么非 tick 触发的 schedule() 调用（比如 block() 导致的）也会重置计数器，导致下一次 tick 的时间片从 0 开始计算——这不会导致功能错误，但会让时间片计量的语义变得混乱。
...
一个微妙但关键的正确性保证：从 `current_ = next` 到 `context_switch(&prev->ctx, &next->ctx)` 之间，中断仍然是关闭的（因为我们可能在中断上下文中执行）。这意味着这两个操作之间不会被另一个定时器中断打断，状态更新是原子的。如果在这里被中断打断，current_ 指向新任务但实际还没切换过去，第二次 tick() 就会在错误的状态上做调度决策——current_ 和实际运行的线程不一致。
...
这里有一个值得强调的初始化顺序约束。run_first() 被调用时，boot_task 的 CpuContext 已经被设置好——ctx.rip 指向调用 run_first() 的那行代码之后的地址，ctx.rsp 指向 boot_task 的内核栈。run_first() 在设置好 current_ 和 g_per_cpu 后，调用 pick_next() 获取第一个真正的任务（不是 boot_task），然后通过 context_switch 从 boot_task 切换到第一个任务。boot_task 本身不会被加入 RoundRobin 队列——它只是一个临时的"启动载体"，用来提供初始的 context_switch 上下文。

**实现约束**: init() 中的初始化顺序很重要——先清空所有状态（包括新增的 idle_task_、tick_count_、current_slice_），再注册调度策略，然后创建 idle 任务，最后设置 initialized_ = true。如果顺序颠倒了——比如先设置 initialized_ = true 再创建 idle 任务——那么如果在 initialized_ 和 idle 任务创建之间有一个 IRQ0 到来（虽然理论上不可能，因为我们还没 sti，但防御性编程不嫌多），tick() 会认为调度器已初始化但 idle 任务还不存在，schedule() 的回退路径会走到 `cli; hlt`。

**踩坑预警**: yield() 简化为调用 schedule() 后，它的行为和 tag 019 不完全一样。在 tag 019 中，yield() 只切换到 RoundRobin 中的下一个任务——如果队列中只有一个任务（自己），yield() 什么都不做。在 tag 020 中，schedule() 的逻辑更复杂——如果当前任务是 Running 状态且 pick_next 返回自己，schedule() 会恢复 Running 并返回，行为和 tag 019 一样。但如果当前任务已经被标记为 Blocked（比如在 yield 之前不小心被 block 了），schedule() 会切到 idle。
...
新增代码的规模大致如下：scheduler.cpp 新增了约 100 行（包括 tick、schedule、block、unblock、remove_task、idle_entry 和 init/yield/exit_current/run_first 的修改），scheduler.hpp 新增了约 15 行声明，加上新增的 PerCPU 全局实例和四个静态成员定义。这是 tag 020 中代码量最大的部分。
...
**schedule() 切换后系统卡死，没有任何输出**: 最可能的原因是 context_switch 的 from 和 to 指向了同一个 Task。在 schedule() 中加一个 kprintf 打印 prev 和 next 的地址和 tid，确认它们确实不同。另一种可能是 next 任务的 CpuContext 没有正确初始化——如果 ctx.rip 指向了一个无效地址，jmp *56(%rsi) 会跳到错误的位置导致 triple fault。检查 TaskBuilder 是否正确设置了 ctx.rip 为线程入口函数。
...
**remove_task() 后任务仍然在运行**: remove_task() 只修改了任务状态和从队列中移除，但不会触发上下文切换。如果你 remove 的是当前正在运行的任务，它还会继续执行直到下一个 tick 触发 schedule()。如果需要立刻切换，应该在 remove_task() 后手动调用 schedule()。exit_current() 就是这么做的——它先从队列中移除当前任务，再寻找下一个任务并切换。

**unblock() 后任务没有被立刻调度**: 这是正常行为。unblock() 只是把任务重新加入 RoundRobin 队列，不做立刻调度。被唤醒的任务会在下一个 tick 或下次 schedule() 时被 pick_next() 选中。如果你需要立刻调度，可以在 unblock() 后调用 schedule()——但这通常不是必要的，因为 20ms 的时间片足够短。
...
**yield() 调用后任务立刻被切回来**: 如果 RoundRobin 队列中只有一个任务（yield 的调用者自己），schedule() 中 pick_next() 返回的就是 prev 自己，schedule() 检测到 next == prev 后恢复 prev 的状态为 Running 并返回。这不是 bug，是正确行为——没有其他任务可以切换时，yield 等于空操作。如果你期望 yield 总是切到不同的任务，需要确保队列中至少有两个任务。

**idle 任务被反复调度，但明明有就绪任务**: 最可能的原因是 add_task() 没有被正确调用——任务的 sched_class 为空，没有被加入 RoundRobin 队列。在 schedule() 中加一行 kprintf 打印 pick_next() 的返回值，如果是 nullptr 说明队列为空。然后检查 add_task() 中是否正确调用了 sched_class->enqueue(task)。

**编译报错 "Scheduler::tick 未定义"**: 检查 scheduler.hpp 中是否声明了 `static void tick()`。如果声明了但链接器仍然报错，检查 scheduler.cpp 中是否有 `void Scheduler::tick() { ... }` 的定义。如果你把 tick() 的实现放在了 scheduler.hpp 中（作为 inline 函数），链接器不会报错——但这样做的风险是 scheduler.hpp 变得过于臃肿。推荐的做法是把所有实现都放在 scheduler.cpp 中。

**运行时日志中 idle 任务被创建了两次**: 说明 Scheduler::init() 被调用了两次。检查 kernel_main 中是否有重复调用 init() 的代码路径。init() 应该只在启动时调用一次。在 init() 的开头加一个 `if (initialized_) return;` 的防御性检查可以防止这种问题。
...
从代码结构的角度看，这一篇最重要的设计决策是把 "是否该切换"（tick）和 "怎么切换"（schedule）拆成两个函数。这种分层让 schedule() 可以被 tick()、block()、yield() 三个不同的调用者复用，避免了重复代码。如果你把计数逻辑和切换逻辑耦合在一起，每个调用点都得写一份几乎相同的判断和切换代码——这和 tag 019 中 yield() 和 exit_current() 有大量重叠代码的问题一样。schedule() 的引入消除了这种重复。

你可以用以下命令验证编译。虽然抢占式调度还不能工作，但内核应该能正常启动：

```bash
cmake --build build --target big_kernel 2>&1 | tail -30
```

如果你在运行中看到任何异常（比如崩溃、输出乱序），说明在修改 scheduler 时引入了回归。最简单的排查方式是暂时注释掉 tick() 和 schedule() 的实现，只保留 init() 和 yield() 的修改——如果问题消失了，说明 bug 在新增的函数中。
