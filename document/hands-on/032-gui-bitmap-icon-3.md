...
**踩坑预警**: 内核端测试用的 Framebuffer 需要从 BootInfo 初始化（物理地址 0x7000），如果 BootInfo 中的帧缓冲区信息不正确（比如在非图形模式的 QEMU 配置下），测试会崩溃。另外，宿主端测试不能直接 include icon_data.hpp（因为 consteval），需要手动定义测试用的图标数据。
