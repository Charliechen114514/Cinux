...
PCID 允许 CPU 为不同的地址空间维护独立的 TLB 缓存（Intel SDM Vol.3A Section 4.10.1），避免每次切换都全部刷新。Linux 从 3.17 版本开始支持 PCID，显著降低了进程切换的开销。
...
这些验证需要两组测试来实现。第一组是 QEMU 集成测试——运行在真实的内核环境中，使用真实的 PMM 和 VMM，测试结果最可信。
第二组是 host 单元测试——在开发机上编译运行，使用模拟的 PMM（位图分配器）和模拟的 VMM（简化的页表遍历），优点是执行速度快、可以用 valgrind 和 AddressSanitizer 检查内存错误。
...
记得在调用测试之前先执行 AddressSpace::init_kernel()。
...
**验证**: 通过 CTest 运行 host 端测试，所有测试应该通过。
