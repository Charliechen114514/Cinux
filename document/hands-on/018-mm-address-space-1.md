...
在设计自己的地址空间抽象之前，我们先看看两个经典 OS 是怎么处理"每进程页表"这个问题的。

xv6（MIT 的教学操作系统，RISC-V 版本）在每次进入内核态时会从用户页表切换到内核的全局页表（`kernel_pagetable`），离开内核态时再切回来。
这种方式更干净（内核页表和用户页表完全分离），但每次系统调用都要做两次页表切换。
另外，xv6 的双页表设计还带来了一个限制——内核无法直接通过用户虚拟地址访问用户内存（因为内核运行在自己的页表上），必须通过 `copyin`/`copyout` 函数手动翻译。

Cinux 选择了另一种方式——和 Linux 一样，把内核映射放到每个进程的页表里，这样进程切换时只需要一次 CR3 写入，而且系统调用和中断处理不需要额外切换页表。两种方式各有取舍，但对于教学操作系统来说，Cinux 的方式更简单，而且在 x86-64 上更自然——因为 x86-64 的 syscall/sysret 指令对页表切换的支持不如 RISC-V 的 S 模式灵活。

Linux 的做法和 Cinux 更像。Linux 用 `mm_struct` 结构管理进程的地址空间，`pgd_alloc()` 分配新的 PGD（Page Global Directory，在 x86-64 上 PGD 就是 PML4），然后通过 `pgd_ctor()` 把内核条目复制到 PGD 的对应位置——这和 Cinux 的构造函数逐条复制 PML4[256..511] 在逻辑上完全一致。进程切换时，`switch_mm()` 把新进程的 PGD 写入 CR3，和 Cinux 的 `activate()` 干的事情一模一样。不过 Linux 的 `mm_struct` 比 Cinux 的 `AddressSpace` 复杂得多——它还包含 VMA 链表、RSS 计数器、mmap 信号量等大量成员。Cinux 的 `AddressSpace` 目前就是 Linux `mm_struct` 的最小子集。

参考：

- Intel SDM Vol.3A Section 4.5.2, pp.4-20 to 4-21（CR3 在四级分页中的角色）
- OSDev Wiki — [Higher Half Kernel](https://wiki.osdev.org/Higher_Half_Kernel)
- xv6 RISC-V `kernel/proc.c` — `proc_pagetable()` 和 `userinit()`
...
这里有一个微妙但重要的细节值得强调：复制 PML4[256..511] 是"浅拷贝"——只复制了 PML4 条目本身的 8 字节 raw 值，没有递归复制 PDPT/PD/PT 这些下层页表页。
所以所有地址空间通过相同的 PML4 条目值（相同的物理地址）指向同一套内核页表结构。这意味着修改内核 PML4 模板中的某个条目会立刻影响所有地址空间的内核映射——这种共享语义在大多数情况下是我们想要的（比如新增 MMIO 映射），但如果某个 AddressSpace 的内核条目被意外篡改，也会影响到所有其他地址空间。
这也是为什么析构函数只释放用户空间的子树（条目 0-255），绝不碰内核空间的条目（条目 256-511）——那些条目指向的页表页不属于当前 AddressSpace，释放它们就是 use-after-free。

参考：

- Intel SDM Vol.3A Section 4.5.4, pp.4-22 to 4-25（四级分页地址翻译流程）
- Intel SDM Vol.3A Table 4-15, pp.4-26 to 4-27（PML4 条目格式）
...
整个类的数据成员只有两个——实例成员 pml4_phys_（当前空间的 PML4 物理地址）和静态成员 kernel_pml4_（启动时保存的内核 PML4 物理地址）。加起来 16 字节。你可以把它类比为一张"身份证"——上面写着一个物理地址，凭借这个地址就能找到一整套四级页表结构。pml4_phys_ 是否为 0 是整个对象状态的"哨兵"：构造成功后非 0，移动后变为 0，析构时也变为 0，所有生命周期操作都先检查它。
...
这个设计决策值得单独拿出来说。拷贝构造和拷贝赋值被 `= delete` 禁止，这不是偷懒——而是安全性的硬要求。

试想如果允许拷贝构造，两个 AddressSpace 对象的 pml4_phys_ 就指向同一个物理 PML4 页。
当第一个对象析构时，它把 PML4 归还给 PMM，第二个对象手里就剩下一个悬空指针。
后续任何 map/unmap/translate 操作都会往已经不属于自己的物理内存上写东西——这是经典的 use-after-free 场景。
更糟糕的是，PMM 的 free_page 可能只是标记物理页为空闲，不清零内容，所以问题不会立刻暴露。
直到 PMM 把这个物理页重新分配出去给别的用途，两个不同的调用者以为各自独占了一页，结果互相踩数据——这种 bug 的排查过程堪称噩梦。

移动语义则是允许的。移动构造和移动赋值把 pml4_phys_ 的所有权转移给目标对象，源对象的 pml4_phys_ 被置 0。
这样源对象析构时检测到 pml4_phys_ == 0 就跳过释放，只有目标对象会执行真正的释放逻辑。
移动操作被标记为 noexcept——这很重要，因为标准库容器（如 std::vector）在 realloc 时会优先使用 noexcept 的移动操作。既然拷贝已经被删了，如果移动不是 noexcept，容器根本无法编译。

在 C++ 中，这种"禁止拷贝、允许移动"的模式是管理独占资源（exclusive ownership）的标准做法。和它类似的例子有 std::unique_ptr、std::thread、std::fstream 等——它们都持有某种不能被多个对象共享的资源，因此都删除了拷贝语义。AddressSpace 持有的"资源"是一个物理 PML4 页，它和 unique_ptr 持有的堆内存本质上是一样的——谁分配谁释放，只能转移不能共享。

### 委托模式：AddressSpace 作为 VMM 的包装器

map、unmap 和 translate 三个操作不需要 AddressSpace 自己实现页表遍历——tag 016 的 VMM 已经把四级页表遍历写得清清楚楚了。
AddressSpace 只需要把自己的 pml4_phys_ 传给 VMM，告诉它"请用我的 PML4 根来操作"就行了。

VMM 的 map/unmap/translate 方法都有一个可选的 pml4_root 参数——传空指针时使用默认的内核 PML4（全局地址空间），传非空指针时使用指针指向的值作为 PML4 根。
所以 AddressSpace 的 map 调用就是 `g_vmm.map(virt, phys, flags, &pml4_phys_)`——传入 pml4_phys_ 的地址，VMM 解引用这个指针拿到 PML4 物理地址，然后从这张 PML4 开始遍历。

这种设计避免了代码重复——页表遍历逻辑只在 VMM 中存在一份。AddressSpace 的三个委托方法加起来不到十行代码，但它们把"在哪个地址空间中操作"这个语义清晰地表达了出来。

这种"所有权包装器 + 委托"的模式在内核代码中非常常见。另一个例子是 Linux 的 `mm_struct`——它持有 PGD 的物理地址（所有权），但页表操作通过底层的 `pgd_xxx` / `pud_xxx` / `pmd_xxx` / `pte_xxx` 内联函数完成（委托）。Cinux 的设计遵循了同样的思路，只是规模更小。
...
**踩坑预警**: `kernel_pml4_` 是一个静态成员变量，它必须在 .cpp 文件中被定义并初始化为 0——如果你忘了在实现文件中写这行定义，链接器会报未定义符号错误（`undefined reference to cinux::mm::AddressSpace::kernel_pml4_`）。另外，删除拷贝构造和拷贝赋值的同时一定要提供移动构造和移动赋值，否则 AddressSpace 对象就无法被放入容器——未来的进程表会需要这个能力。如果你尝试在测试代码中拷贝一个 AddressSpace 对象（比如 `auto as2 = as1;`），编译器会报 "use of deleted function" 错误——这正是我们想要的保护效果。
...
**验证**: 修改 main.cpp 后重新编译运行内核，在串口输出中应该能看到类似 `[AS] Kernel PML4 saved at phys 0x...` 的日志。这个物理地址应该和之前 VMM 初始化时输出的 PML4 地址一致——通常是 0x10000 或 0x20000 附近的一个 4KB 对齐地址。如果日志显示 `Kernel PML4 saved at phys (nil)` 或 0x0，说明 read_cr3() 返回了 0，这几乎不可能发生——检查 main.cpp 中的调用顺序，确认 init_kernel() 是在 VMM 初始化之后调用的。

**init_kernel() 只能调用一次**: 虽然当前的实现没有防重复调用的保护，但重复调用会覆盖之前保存的值。如果未来需要更严格的保障，可以在函数内部加一个 `static bool initialized = false;` 标志位，重复调用时直接 panic。不过在当前阶段，只要把调用安排在 kernel_main() 的确定位置，就不会有重复调用的问题。
...
在这个阶段，你还可以做一些额外的验证来确保一切正常：
1. 确认 `[AS]` 日志输出的物理地址和 VMM 初始化日志中的 CR3 地址一致（VMM 的 init 函数通常会打印它使用的 PML4 地址）。
2. 确认 Heap 初始化（Step 10）和 Framebuffer 初始化（Step 11）仍然正常完成——这说明 init_kernel() 的插入没有破坏任何后续操作。
3. 如果你有 GDB 调试环境，可以在 init_kernel() 处设断点，检查 read_cr3() 的返回值是否和预期一致。这个地址通常是 0x10000 或 0x20000 附近的一个 4KB 对齐值。
...
**启动序列步骤编号变化**: 插入 init_kernel() 之后，原来的 Step 9（Heap 初始化）变成了 Step 10，后续步骤也需要顺延编号。这个编号变化不影响运行时行为，但如果后续代码注释中引用了步骤编号，需要同步更新。建议在 main.cpp 的注释中标注清楚每一步的依赖关系，方便日后维护。

**Higher-half 映射的前提**: init_kernel() 读取 CR3 后保存的是物理地址，但后续构造函数需要通过 phys_to_virt() 把物理地址转换成虚拟地址来访问页表内容。这个转换依赖内核启动时建立的 higher-half 直接映射——物理地址 phys 对应的虚拟地址是 phys + KERNEL_VMA。如果 higher-half 映射不完整（比如没有覆盖到 PML4 所在的物理页范围），后续构造函数访问新分配的 PML4 时就会触发 page fault。在 Cinux 的当前实现中，VMM 初始化时已经建立了完整的 higher-half 映射，所以这不是问题——但理解这个前提条件对于排查未来的 bug 很重要。
...
回顾一下我们做的设计决策：
- **Higher Half 策略**：PML4 的用户空间（条目 0-255）每个进程独立，内核空间（条目 256-511）所有进程共享。这是 x86-64 canonical 地址空间的天然分割，也是 Linux 采用的经典设计。
- **浅拷贝而非深拷贝**：构造函数只复制 PML4 条目的 raw 值，不递归复制下层页表页。所有地址空间共享同一套内核页表结构，修改一处全局生效。
- **禁止拷贝**：两个 AddressSpace 不能共享同一个物理 PML4，从编译期杜绝 use-after-free。
- **显式保存内核 PML4**：用 init_kernel() 静态方法在启动时保存 CR3，而不是在构造函数中读取，确保模板来源确定且一致。
- **委托 VMM**：AddressSpace 不自己实现页表遍历，而是把 pml4_phys_ 传给 VMM，复用已有的 map/unmap/translate 逻辑。
