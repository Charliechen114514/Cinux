...
如果你是从 002 章一路走过来的，你可能已经配过两次 GDT 了——002 章在 Bootloader 的 boot.S 里配了一次（实模式切保护模式），007 章在 mini kernel 的 entry.S 里又配了一次（从 mini kernel 自己管理的内存建立 GDT）。为什么大内核还要配第三次？

原因有三点。第一，Bootloader 的 GDT 在实模式切换保护模式时起作用，但它存放在 Bootloader 的代码段里（低地址区域），大内核运行起来后无法可靠地访问它。第二，mini kernel 的 GDT 也是放在低地址区域，而大内核运行在 higher-half 虚拟地址空间 `0xFFFFFFFF80000000`，GDT 的 GDTR.base 必须指向大内核能直接访问的地址。第三，大内核需要更完整的 GDT——包括用户态段和 TSS，这些是 mini kernel 不需要的。

所以每次"换壳"（从 Bootloader 到 mini kernel 到大内核）都需要重新配一次 GDT。好消息是，这是最后一次了——后续章节不会再次重建 GDT，只会在现有的 GDT 上修改（比如填充 TSS 的 RSP0 和 IST 字段）。
...
**问题: GDT 加载后系统正常但后续 IDT 加载时 triple fault**
检查 GDT 中 kernel code64 描述符的 access byte 是否正确。最常见的错误是忘了设 LongMode 标志（L=1），导致 CPU 以为这是一个 32 位代码段，后续 `iretq` 指令在 32 位模式下不合法。另一个可能的原因是 data 段的 L 位被错误地置为 1——在 long mode 下数据段的 L 位必须为 0，否则 CPU 行为未定义。

**问题: 编译时 static_assert 失败（sizeof(Entry) != 8 或 sizeof(TaskStateSegment) != 104）**
检查结构体定义是否缺少 `[[gnu::packed]]` 属性。没有 packed 的话编译器会在 `access`（uint8_t）和 `flags_limit_high`（uint8_t）之间插入 padding 字节，导致结构体超过 8 字节。TaskStateSegment 同理——如果 `reserved0`（uint32_t）后面没有 packed，编译器可能插入 4 字节 padding 使 `rsp[3]` 对齐到 8 字节边界。
