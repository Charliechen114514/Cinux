...
**验证**：编译后检查大内核测试是否正常运行（不是 "not a real kernel" 退出）。运行 `make run-kernel-test`，确认测试数量不为 0。
...
**设计思路**：定义常量 USER_ABI_RSP_OFFSET = 8，在传给 jump_to_usermode 的 RSP 参数上减去这个值。同时用 static_assert 编译期检查对齐：(USER_STACK_TOP - 8) % 16 == 8。

**踩坑预警**：这个值是从 usermode.hpp 移到 usermode.cpp 内部匿名命名空间的——不要在头文件里定义，避免其他地方误用。
...
用户态 libc 的 syscall 封装是一个薄层——一个辅助函数用内联汇编执行 syscall 指令（RAX=syscall 号，RDI/RSI/RDX 传参，输出 "=a"(ret)），clobber 列表包含 rcx、r11、memory。然后用这个辅助函数包装 sys_read、sys_write、sys_exit、sys_yield。
...
**设计思路**：不再用硬编码的 kUserCode[] 数组，而是用 extern 声明 _binary_hello_bin_start 和 _binary_hello_bin_end，计算大小后拷贝到用户态代码页。同时设置 GS base 数据页：分配一个物理页，gs:0 存内核栈指针，通过 wrmsr 写入 IA32_KERNEL_GS_BASE。
