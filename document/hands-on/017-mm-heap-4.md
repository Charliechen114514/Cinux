...
本章涉及的文件比较分散：`kernel/arch/x86_64/crt_stub.cpp`（new/delete 重定向）、`kernel/main.cpp`（启动序列集成）、`test/unit/test_heap.cpp`（host 端测试，866 行）和 `kernel/test/test_heap.cpp`（QEMU 端测试，301 行）。
...
你可能会问：为什么不直接用 `g_heap.alloc()` 和 `g_heap.free()`？有几个重要原因。第一，C++ 的 RAII 模式依赖 new/delete——当你写 `auto p = new MyClass()` 时，编译器自动调用 operator new 分配内存然后调用构造函数；写 `delete p` 时先调用析构函数再调用 operator delete。如果不接管，这些语言特性就无法使用。第二，标准库容器内部的内存分配都通过 operator new——接管它意味着所有容器自动使用我们的堆分配器。第三，全局对象的动态初始化也可能触发 operator new——不接管的话这些构造函数会触发 halt。
...
两层测试互补——host 端覆盖大量边界情况和压力测试（25+ 个用例），QEMU 端验证真实环境的集成正确性（10 个核心用例）。这种策略和 tag 015/016 中 PMM/VMM 的测试方式一脉相承。
...
`validate_free_list()` 的"环检测"机制值得一提——它记录遍历步数，超过 10000 就判定链表有环。这是一种简单但有效的策略：正常情况下空闲链表的块数不会超过几十个，如果遍历了 10000 步还没到链表末尾，几乎可以确定 next 指针形成了一个环。链表有环通常是由于 coalesce 中摘除节点时忘记清除 next 指针导致的。

本章涉及的文件分散在四个位置，但每个文件的修改量都不大。crt_stub.cpp 从"halt on use"变为"delegate to g_heap"是最大的逻辑变化，但每个重载函数体只有一行代码。main.cpp 只增加了 3 行（堆初始化调用）。测试文件虽然很长（866 行 + 301 行），但大部分是重复的测试用例模板，核心逻辑并不复杂。
...
**设计思路**: 需要实现十个重载版本（4 个 new + 6 个 delete）。四个 new 版本：`operator new(size_t)` 调用 `g_heap.alloc(size)`；`operator new[](size_t)` 调用 `g_heap.alloc(size)`；`operator new(size_t, std::align_val_t)` 调用 `g_heap.alloc(size, static_cast<size_t>(align))`；`operator new[](size_t, std::align_val_t)` 调用 `g_heap.alloc(size, static_cast<size_t>(align))`。六个 delete 版本：`operator delete(void*)`、`operator delete(void*, size_t)`（sized）、`operator delete[](void*)`、`operator delete[](void*, size_t)`（sized array）、`operator delete(void*, std::align_val_t)`（aligned）、`operator delete(void*, size_t, std::align_val_t)`（sized aligned）——全部调用 `g_heap.free(ptr)`，size 和 alignment 参数被忽略。
...
为什么要实现全部 6 个 delete？GCC 从某个版本开始会在 `-O2` 下生成 sized delete 的调用（如果存在对应的重载）。如果不提供 sized delete 的实现，链接器在启用 sized delete 时会报 `undefined reference`。aligned delete 同理——如果定义了 aligned new 但不定义 aligned delete，编译器在某些情况下会生成未定义的 delete 调用。六个重载全部实现保证了在任何编译选项下都不会出现链接错误。

**验证**: 在内核中尝试用 `new` 分配一个对象，然后用 `delete` 释放。比如在某处写 `auto* p = new int(42); *p; delete p;`——如果不触发 halt，说明重定向成功。更正式的验证在后续的测试中进行。

你也可以用 `nm` 或 `objdump` 检查编译后的目标文件，确认 operator new 指向了调用 `g_heap.alloc` 的实现而不是死循环。命令示例：`x86_64-elf-nm build/kernel/big_kernel_common.a | grep "operator new"`。如果看到的地址指向一个 `cli; hlt` 循环，说明 crt_stub.cpp 没有被正确编译或者旧的目标文件被缓存了——尝试 clean build。
...
为什么不直接链接 `heap.cpp`？因为直接链接会引入所有内核头文件的依赖（`kprintf`、`pmm.hpp`、`vmm.hpp`），这些头文件又依赖 `paging_config.hpp`、`serial.hpp` 等，编译依赖链非常复杂。镜像实现虽然需要维护两份代码，但完全自包含——不需要任何内核头文件，编译速度快，可以用 GDB 单步调试。
...
QEMU 端测试的一个重要价值是验证端到端的正确性——从 alloc 到页表映射到物理内存访问的整条链路。如果 VMM 的映射有误（比如映射到了错误的物理页），或者页表标志位不对（比如忘了设 writable），QEMU 端测试会立刻触发 page fault，而 host 端测试（使用 calloc，没有页表映射）完全无法发现这种问题。这就是为什么需要两层测试的原因。
...
注意 `test_heap` 目标的 include 路径需要能找到 `test_framework.h`。如果 include 路径有误，编译时会报找不到头文件的错误。其他 include 路径（如内核头文件目录）对 host 测试不需要，因为 TestHeap 是自包含的。
...
所有测试（包括之前的 PMM、VMM、kprintf 测试）加上新增的 heap 测试应该全部通过。如果某个测试失败，ctest 会打印失败用例的名称和断言信息，帮助定位问题。

## 本章小结

本章完成了三件事：C++ new/delete 的重定向（10 个重载版本全部委托给 Heap）、堆在启动序列中的位置（VMM 之后、Framebuffer 之前）、以及双层测试体系（host 端 866 行 + QEMU 端 301 行）。tag 017 至此全部完成——Cinux 的内存管理栈终于完整了：PMM 管物理页，VMM 管虚拟映射，Heap 管任意大小的动态分配。
...
**构建失败 "undefined reference to operator delete(void*, unsigned long, std::align_val_t)"**: 这说明编译器生成了 aligned sized delete 的调用但链接器找不到实现。检查 crt_stub.cpp 是否包含了所有 6 个 delete 重载版本。如果你只实现了基本的 delete 而没有实现 aligned 和 sized 版本，就会触发这个链接错误。
...
**QEMU 测试中 double-free 没有被捕获**: QEMU 端测试目前没有 double-free 测试用例，因为 double-free 只会打印 kprintf 警告不会 crash。要验证 double-free 检测，可以临时在 QEMU 测试中手动调用两次 free 同一个指针，然后检查串口输出是否包含 "Double-free detected"。

**host 测试和 QEMU 测试结果不一致**: 如果 host 测试通过但 QEMU 测试失败（或者反过来），说明两边的算法实现有差异。逐行对比 TestHeap 和 Heap 的 alloc/free/coalesce 逻辑。最容易出偏差的地方是 coalesce 中"链表头摘除"的处理和 alloc 中"usable < size"的边界检查。

## 本章小结

| 概念 | 要点 |
|------|------|
| operator new/delete | 十个重载版本全部重定向到 g_heap.alloc/free |
...
| QEMU 测试地址 | 0xFFFFFFFF80100000（不同于正式内核的地址，避免和测试内核代码段重叠） |
| 总代码量 | heap.hpp(126行) + heap.cpp(353行) + crt_stub.cpp(修改) + 测试(1167行) |
