...
### Step 2: launch_first_user() — 构建用户地址空间

**目标**：创建一个 AddressSpace，映射用户代码页（0x400000）和用户栈页（0x7FFFFF000 附近），复制 identity mapping，激活地址空间，设置 TSS.RSP0，然后调用 jump_to_usermode。

**设计思路**：整个流程分九步：(1) 创建 AddressSpace；(2) 分配用户代码物理页；(3) 在 USER_ENTRY_BASE (0x400000) 映射代码页；(4) 通过高半区直接映射把机器码字节复制到代码页；(5) 分配 USER_STACK_PAGES (4) 个物理页，映射到栈区域；(6) 复制内核 PDPT 的 identity mapping 到用户 PDPT；(7) activate() 切换 CR3；(8) 设置 TSS.RSP0 为当前内核栈；(9) 调用 jump_to_usermode。
...
==== EXCEPTION: #GP (vector 13) ====
  RIP   = 0x400000   CS  = 0x001b
[EXCEPTION] #GP at RIP=0x400000 from user mode (Ring 3)
[EXCEPTION] Privileged instruction executed in Ring 3 -- protection works!
```

### Step 3: 修改 kernel_main 集成

**目标**：在 kernel_main 中调用 usermode_init() 和 launch_first_user()，替换掉原来的 producer-consumer 演示。

**设计思路**：从 kernel_main 中移除 021 tag 的调度器和同步演示代码，替换为 usermode 初始化和首次用户态启动。初始化顺序保持不变（GDT → IDT → PIC → PIT → PMM → VMM → AddressSpace → Heap → FB → Console → Keyboard），在键盘初始化之后、中断使能之后调用 usermode_init()，然后调用 launch_first_user()。

**验证**：完整构建运行后，看到上述的 Ring 3 #GP 输出。CS=0x1B 确认 CPU 在 Ring 3，RIP=0x400000 确认用户代码正确执行到了 cli 指令。
...
3. **CS=0x13 而不是 0x1B**：说明 STAR MSR 写入有误，SYSRET 计算出了错误的 CS。回顾第一章的 shlq 位数讨论。
...
| #GP 验证 | CS=0x1B(Ring 3) + RIP=0x400000 = 特权隔离成功 |
