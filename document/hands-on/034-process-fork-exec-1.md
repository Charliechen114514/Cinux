...
**踩坑预警**: phoff 是 program header table 在文件中的偏移量，不是虚拟地址。如果 phoff 大于文件大小，说明 header 声明的 program header table 根本不在文件里，这是一个损坏的 ELF 文件。

**验证**: 写一个 host 端单元测试，构造一个合法的 ELF header buffer 验证通过，然后分别改掉 magic / class / machine / type 验证返回对应错误码。
...
**实现约束**: 类名叫 PidAllocator，放在 `proc` 命名空间。常量 PID_MAX=256、PID_NONE=0。需要 alloc()、free(pid)、is_allocated(pid)、count() 四个方法。私有成员是一个 bool 数组（大小 PID_MAX+1，因为索引 0 到 256）和一个 int next_hint_。构造函数把所有标记清 false，hint 设为 1。分配时从 next_hint_ 开始线性扫描 PID_MAX 个位置，用取模方式回绕到 1（跳过 0）。
...
**验证**: Host 端测试分配递增、释放后复用、分配耗尽返回 0、释放无效 PID 是 no-op、count() 计数正确。
...
**实现约束**: 函数签名遵循统一的 6 参数格式 `int64_t sys_xxx(uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t)`， getpid 和 getppid 不需要参数，全部忽略。注册到对应的 SyscallNr 枚举值。

**验证**: 在 QEMU 内核测试中创建一个任务，设置它的 pid 和 ppid，然后通过 syscall 调用 getpid/getppid 验证返回值。

## 构建与运行

构建命令和之前一样：`cmake --build build` 然后 `make run`。如果只想跑 host 端测试：`build/test/unit_test`。QEMU 内核测试：`make run-kernel-test`。

## 调试技巧

1. **ELF 验证失败**: 如果 execve 报 ELF validation failed，先打印 e_ident 的前 16 字节看是不是真的是 ELF 文件。常见错误是把目录文件当作可执行文件传给了 execve。

2. **PID 耗尽**: 如果 fork 返回 -1 并且串口打印 "PID allocator exhausted"，说明有进程在退出后没有调用 free() 回收 PID。检查 waitpid 和 exit 路径是否正确释放了 PID。
...
| 组件 | 关键设计 | 对应源文件 |
|------|----------|-----------|
| Elf64_Ehdr | 64 字节 packed struct, static_assert 校验 | elf_types.hpp |
| Elf64_Phdr | 56 字节 packed struct, PT_LOAD 段描述 | elf_types.hpp |
| validate_elf_header | 顺序检查 8 个字段 | elf_types.cpp |
| PidAllocator | hint-based 线性扫描, PID_MAX=256 | pid.hpp/cpp |
| sys_getpid / sys_getppid | 直接读 TCB 字段 | syscall/sys_getpid.cpp, sys_getppid.cpp |
