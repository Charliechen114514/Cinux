...
需要注意的是，轮询模式的 `putc()` 是阻塞式的——调用者会一直等在那里直到字节发送完毕。这意味着如果你在中断处理程序（ISR）里调用 `kprintf`，而恰好上一次发送还没完成，ISR 就会被卡住，整个系统都会卡住。所以轮询模式的串口输出不适合在高频中断里大量使用。如果你将来需要在中断上下文中输出调试信息，可以考虑用 QEMU 的 debugcon（端口 0xE9）作为替代——debugcon 的写入是即时的，不需要轮询。
...
还有一个容易被忽略的细节：UART 的 LSR 寄存器是只读的——你不能通过写 LSR 来强制设置 TX_READY。这意味着如果你想把 THR 的状态"重置"为空，唯一的办法就是等硬件自己把字节发完。在真实硬件上，115200 波特率下发送一个字节（10 个 bit）大约需要 87 微秒——看起来很短，但如果一次性发送几千个字符（比如 dump 一个大型数据结构），累计的等待时间就不容忽视了。QEMU 的虚拟 UART 发送是即时的，所以在虚拟环境下你不会感受到这个延迟。
...
如果你在 Windows 的 WSL 环境下开发，串口输出可能有额外的延迟——这是因为 WSL1 的 I/O 需要经过 Windows 内核中转，WSL2 则使用真正的 Linux 内核，I/O 性能更好。如果串口输出看起来一卡一卡的，尝试切换到 WSL2 或使用 `-serial file:serial.log` 把输出写到文件里而不是直接显示到终端。

如果你想同时看到 debugcon 的输出（用于排查内核启动最早期的问题），可以在 QEMU 参数里加 `-debugcon file:debug.log`，这样所有往端口 0xE9 的 outb 操作都会被记录到 debug.log 文件中。

如果你在真机（而非 QEMU）上测试，需要注意两点：第一，真机上的 UART 波特率需要通过 DLAB 分频器手动设置（QEMU 默认 115200 不需要设），否则输出会乱码。第二，真机可能需要额外的 Modem 握手信号（CTS/DSR）才能发送数据，而 QEMU 的虚拟 UART 不检查这些信号。
...
**init() 之后第一条 kprintf 就崩溃**

如果你发现调用 `kprintf_init()` 之后的第一次 `kprintf` 调用就崩溃，且崩溃点在 `putc` 的轮询循环里，那最可能的原因是 `init()` 根本没有被调用——检查 `kernel_main` 里 `kprintf_init()` 的调用是否在 `kprintf` 之前。另一个可能性是 Serial 对象的地址被踩了——在 `kprintf_init()` 之后立刻用 GDB 查看 `g_serial` 对象的内存布局，确认 `base_port_` 字段没有被别的代码覆盖。
...
| 全局 Serial 对象 | 构造不初始化 | 避免 C++ 全局构造函数中的硬件 I/O |
| 轮询阻塞 | `putc()` 会等待 TX_READY | 不适合在高频 ISR 中大量使用 |
| DLAB 位 | LCR bit 7 | QEMU 下无需设置波特率，DLAB 保持 0 |
