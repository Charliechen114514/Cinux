...
系统调用号我们参考 Linux x86-64 的约定，用 enum class 定义：SYS_read=0, SYS_write=1, SYS_yield=24, SYS_exit=60。之所以照搬 Linux 的编号，是为了将来移植用户程序时减少修改量。Linux 之所以选这些特定的数字（比如 exit 是 60 而不是 2），纯粹是历史原因——这些数字从 Linux 最早的 x86-64 移植版本就定下来了，为了保证二进制兼容性一直没变过。
...
int64_t sys_write(uint64_t fd, uint64_t buf_virt, uint64_t count,
                  uint64_t, uint64_t, uint64_t) {
    if (buf_virt >= USER_ADDR_MAX) return -1;
    if (fd != 1) return -1;
...
sys_write 是三个 handler 中安全设计最值得说的。第一道防线是地址验证：buf_virt 必须 < 0x800000000000。这个值是 x86-64 canonical address 的分界线——低于此值的是用户空间（低 47 位寻址），高于此值的是内核空间。如果用户程序传入一个内核地址（比如 0xFFFFFFFF80000000），这个检查直接拦截。第二道防线是 fd 检查——目前只支持 fd=1（stdout）。

说句实话，这个验证是最基本的。它只检查地址范围，不检查该地址是否真的在当前进程的页表中有映射。Linux 用 copy_from_user() 配合 SMAP/SMEP 硬件机制做更完善的检查——如果用户传入一个未映射的地址，copy_from_user 会触发缺页异常并通过 fixup 表优雅地返回错误。Cinux 目前的实现是够用的，但离生产级安全还有距离。
...
把 sys_write、sys_exit、sys_yield 注册到分发表后，在 kernel_main 里调用 syscall_init 然后注册三个 handler。编译运行后串口应显示 syscall MSR 配置成功的日志。此时还没有用户程序调用 syscall——端到端的验证要等到下一章。
