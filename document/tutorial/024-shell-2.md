...
xv6 也有自己的 printf 实现（printf.c），支持类似的格式说明符集。两者的设计思路几乎相同——内部缓冲区 + sys_write 刷新。区别在于 xv6 的 printf 支持多 fd（通过第一个参数指定），而 Cinux 的 printf 只输出到 fd=1（stdout）。两者的格式说明符子集基本相当——都不支持 %f 和字段宽度。
