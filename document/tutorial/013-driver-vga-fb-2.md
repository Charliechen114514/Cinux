...
字体文件在编译时嵌入内核，所以不依赖运行时的文件系统。我们使用 IBM PC 经典的 8x16 位图字体（CP437 编码），每个字符是 8 像素宽 × 16 像素高的单色位图。工具链是 GAS（GNU Assembler）+ Python 3，嵌入方式是 `.incbin` 伪指令。PSF2 格式的魔数是 0x864AB572，整个字体文件只有 4128 字节。
