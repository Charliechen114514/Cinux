...
这里有一个容易混淆的地方：P=0 不一定是坏事。对于 demand paging 来说，P=0 恰恰是"这个页还没分配，请操作系统补上"的信号，是正常的工作流。只有 P=1 的情况——页存在但权限不够——才是真正的"非法访问"，需要向进程发送 SIGSEGV 或者内核 panic。OSDev Wiki 的 [Page Fault](https://wiki.osdev.org/Page_Fault) 页面对这些位域有非常清晰的总结，建议配合 Intel SDM 一起看。
...
接下来有一个关键的判断：`is_user_vaddr(fault_addr)` 检查故障地址是否属于用户空间。如果是用户空间地址，映射标志需要加上 `FLAG_USER`——否则 Ring 3 的代码后续访问这个页时又会触发权限违反的 #PF，陷入死循环。这比"一律内核权限"的做法更完善。

然后是分配和映射。这里用了 PMM 和 VMM 的**无锁版本**——`alloc_page_locked()` 和 `map_nolock()`。注释里解释了原因：#PF handler 运行在中断门下（IF=0），此时不可能有并发的 VMM/PMM 访问，如果强行拿锁反而可能在递归缺页时死锁。`map_nolock` 正是上一篇中 VMM 类设计的无锁内部方法。

还有一个值得注意的细节：`read_cr3()` 读取当前的 CR3 值传给 `map_nolock` 的 `pml4` 参数。这确保了 demand paging 操作的是当前正在使用的页表根，而不是 VMM 保存的默认内核 PML4——在将来实现进程切换后，缺页时 CR3 可能指向不同进程的 PML4。

如果两个操作都成功，直接 `return`——CPU 执行 IRETQ 回到触发异常的那条指令重新执行。此时页表已经建好了，MMU 做地址翻译能找到有效的 PTE，指令正常通过。如果 PMM 分配失败（物理内存耗尽）或者映射失败，代码会调用 `free_page_locked` 释放刚分配的物理页，然后落入下方的 fatal 路径。这种"分配失败就释放、不泄漏"的处理比直接 panic 稍微优雅一些。

关于锁的问题值得多说一句。这里用的是 `alloc_page_locked()` 和 `map_nolock()`——PMM 和 VMM 的无锁版本。原因在于 #PF handler 是通过中断门进入的——中断门会自动清除 EFLAGS.IF（即 IF=0），这意味着在 handler 执行期间不会有一个中断来打断我们、抢走锁。所以在单核 QEMU 环境下，不存在并发的 PMM/VMM 访问，拿锁反而是多此一举，甚至在递归缺页（handler 本身触发了新的缺页）时会死锁。

当然，到了多核环境这个故事就完全不同了——到那时我们需要 spinlock 保护页表操作，而且必须用 `try_lock` 而不是 `lock` 来避免死锁。原因在于多核系统上，#PF handler 可能在一个核上执行页表操作的同时，另一个核上的内核代码也在修改同一个页表。没有锁保护，两个核可能同时修改同一个 PTE，导致其中一个核的修改被覆盖——这是一个非常隐蔽的数据竞争 bug，而且因为它涉及硬件级的并发，复现和调试的难度都极高。但那是后话了——单核 QEMU 环境下我们暂时不用操心这个问题。
...
说完了实现和测试，我们站远一点看看 Cinux 当前的 demand-paging 策略在整个操作系统光谱上处于什么位置。对比不是为了分高下——三个系统处在完全不同的演进阶段——而是为了看清"Cinux 当前做到了哪一步，下一步该往哪走"。
...
- OSDev Wiki: [Page Fault](https://wiki.osdev.org/Page_Fault) — #PF 异常的错误码位域详解
- OSDev Wiki: [TLB](https://wiki.osdev.org/TLB) — TLB 缓存机制和刷新策略
- xv6 RISC-V `trap.c` 和 `vm.c`：[xv6-riscv 源码](https://github.com/mit-pdos/xv6-riscv/blob/master/kernel/trap.c) — xv6 的缺页处理和地址合法性校验
