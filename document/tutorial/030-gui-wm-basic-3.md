...
前两章我们分别搭好了输入管道（鼠标驱动+事件队列）和输出框架（Window+WindowManager+compositing），但它们还是各干各的——鼠标事件躺在队列里没人吃，窗口管理器空转不处理输入。这一章要把这两半串起来，让鼠标事件真正驱动窗口行为：点击 raise、标题栏拖拽、关闭按钮销毁。同时我们还要把整个 GUI 的初始化集成到内核启动序列里，处理好"哪个阶段能做什么"的依赖关系。

## 环境说明

- 平台：x86_64 (QEMU, PS/2 鼠标 + VBE framebuffer)
- 屏幕分辨率：1024x768x32bpp
- PIT 频率：100Hz（每 10ms 一次 tick）
- GUI 刷新率：与 PIT 相同，最高 100fps

## 第一步——鼠标事件的状态机

窗口管理器处理鼠标事件本质上是一个简单的状态机。空闲状态下收到 MouseDown 时，我们先做 hit test——从最顶层窗口向下遍历，找到第一个包含点击坐标的可见窗口。为什么要倒序遍历？因为上面的窗口遮挡下面的，从底层开始遍历会让被遮挡的窗口"偷"到点击事件。

找到了被点击的窗口之后，还要看点击落在窗口的哪个部位。如果命中右上角的关闭按钮（14x14 红色方块），就销毁窗口。如果命中标题栏（y 坐标在窗口顶部 20px 范围内），就进入拖拽模式。不管点在哪里，都先 raise 把窗口提到最顶层。

拖拽的关键是记录偏移量。当用户点击标题栏时，鼠标通常不在窗口左上角——可能在标题栏中间的某个位置。我们记录下"点击时鼠标相对于窗口左上角的偏移"，然后在后续的 MouseMove 事件中，用当前鼠标坐标减去偏移量得到窗口的新位置。没有这个偏移量，窗口会在点击的瞬间跳到鼠标位置，用户体验非常差。
...
MouseUp 事件清除拖拽标志，回到空闲状态。整个状态机就这三个状态转换——简单但完整。

对比 SerenityOS 的窗口拖拽实现：它支持标题栏拖拽、边框 resize、窗口最大化/最小化动画，还有"吸附到屏幕边缘"的磁吸效果。Cinux 只实现了最基础的标题栏拖拽。对比 X11 的做法：拖拽是由窗口管理器进程处理的（通过 XGrabPointer + ConfigureNotify 事件），而不是内核直接操作——这是用户空间窗口管理器的好处之一，可以独立于内核升级和替换。

## 第二步——PIT tick 驱动的 GUI 事件循环

我们的 GUI 没有独立的事件循环线程。取而代之的是借用 PIT 定时器的 tick 回调——每 10ms 一次的定时器中断里，我们做三件事：
...
这种设计有一个明显的限制：GUI 帧率被 PIT 频率绑定了。100Hz 的 PIT 给我们最多 100fps。如果 compositing 耗时超过 10ms（在 1024x768 分辨率下不太可能，但更高分辨率就可能），tick 回调会堆积，导致中断延迟。
...
临时解决方案是在 QEMU 启动参数里加 `-usb -device usb-tablet`。USB tablet 设备通过 HID Absolute Pointer 报告绝对坐标，QEMU 用它来同步宿主和客户机光标。不过我们的内核还没有 USB 驱动，这个参数主要是让 QEMU VNC 层面处理光标同步。

长期方案有两个方向：一是实现 USB HID 驱动读取 USB tablet 的绝对坐标，二是实现 PS/2 鼠标的 Intellimouse 扩展获取滚轮支持（但对绝对坐标没有帮助）。USB HID 是更彻底的解决方案。
