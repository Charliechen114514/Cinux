...
void Canvas::blit(int32_t dst_x, int32_t dst_y, Canvas& src,
    ...
    uint32_t dst_ppr = pitch_ / 4;
    uint32_t src_ppr = src.pitch_ / 4;
    ...
    for (uint32_t row = 0; row < h; row++) {
        ...
        for (uint32_t i = 0; i < col_count; i++) {
            ...
            back_buf_[dst_row * dst_ppr + dst_col] =
                src.back_buf_[src_row * src_ppr + src_col];
        }
    }
}
```

这段代码的核心复杂性在于处理窗口被拖到屏幕边缘外的情况。dst_x 和 dst_y 是 int32_t——负数意味着窗口的左边缘或上边缘在屏幕外。行方向的处理比较直接：dst_row < 0 时跳过（还没到可见区域），dst_row >= height_ 时 break（已经过了可见区域）。列方向需要更精细的处理：当 dst_x 为负数时，计算 col_skip = -dst_x，然后从源区域的第 col_skip 列开始拷贝，目标从第 0 列开始。

这种「在底层 API 中处理裁剪」的设计选择是有意为之的——窗口管理器只需要传入实际坐标（可能为负），不需要自己做复杂的可见区域计算。这和 SerenityOS 的 Painter::blit() 以及 Linux DRM 的 plane composition 类似——底层负责裁剪，上层只管画。
...
- OSDev Wiki: [Drawing In a Linear Framebuffer](https://wiki.osdev.org/Drawing_In_a_Linear_Framebuffer) — fillrect 优化和位图渲染
