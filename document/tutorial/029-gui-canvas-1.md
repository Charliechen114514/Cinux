...
public:
    ...
    // void init(uint32_t w, uint32_t h); // 离屏 Canvas — tag 030 添加
...
从 Framebuffer 获取维度信息，分配 `width_ * height_` 个 uint32_t 作为后备缓冲区，然后用 memfill32 清零为黑色。对于 1024x768 的屏幕，这会分配大约 3MB 的内存（1024 * 768 * 4 = 3,145,728 字节）——这个大小直接引发了后面要讲的堆安全修复。
...
- OSDev Wiki: [Drawing In a Linear Framebuffer](https://wiki.osdev.org/VBE) — 像素寻址、fillrect 优化、双缓冲原理
- OSDev Wiki: [VESA BIOS Extensions](https://wiki.osdev.org/VBE) — linear framebuffer、pitch 概念
