...
// Stack (follows Framebuffer — key fix: stacks come AFTER MMIO)
...
修复的核心在 `KMEM_STACK_BASE = KMEM_FB_BASE + KMEM_FB_SIZE`——栈从 MMIO 和 Framebuffer 之后开始。在旧代码中，栈的起始地址和 MMIO 相同；现在栈在 MMIO 之后 18 MB 的位置，永远不会重叠。
...
- OSDev Wiki: [MMIO](https://wiki.osdev.org/MMIO) — MMIO 映射注意事项（cache disable、地址预留）
- OSDev Wiki: [Memory Map](https://wiki.osdev.org/Memory_Map) — 物理内存布局
