...
chdir 是 Unix 最古老的 syscall 之一——它的编号是 12，在 Linux syscall 表中排第 12 位（紧跟着 close 的 3 和 stat 的 4 之后）。在 V7 Unix 中就已经存在了。Cinux 的 sys_chdir 实现非常直白：
...
sys_fstat 通过文件描述符查找 inode——省去了路径解析和 VFS 查找两步，直接从全局 FD 表拿到 inode 指针。后半段（调用 stat、拷贝结果）完全一样。
...
- ext2 disk inode format: https://www.nongnu.org/ext2-doc/ext2.html#inode-table
