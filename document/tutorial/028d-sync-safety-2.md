...
```cpp
// PIT
static lib::Atomic<uint64_t> tick_count_{0};
tick_count_.fetch_add(1, lib::MemoryOrder::Relaxed);
return tick_count_.load(lib::MemoryOrder::Relaxed);

// TaskBuilder
lib::Atomic<uint64_t> next_tid{1};
task->tid = next_tid.fetch_add(1, lib::MemoryOrder::Relaxed);
```

全部使用 `MemoryOrder::Relaxed`——我们只需要原子性，不需要与其他操作建立顺序保证。在 x86 上，`fetch_add(relaxed)` 被编译为 `lock xadd` 指令，`load(relaxed)` 被编译为普通的 mov 指令（x86 的所有加载都有 acquire 语义），开销极小。
...
- Intel SDM: Vol.3A Section 9.1 — Locked Atomic Operations，LOCK 前缀与原子指令语义
...
- Linux: [mutex-design.rst](https://docs.kernel.org/locking/mutex-design.html) — Linux mutex 快速路径/慢速路径/乐观自旋设计
