...
uint64_t PDPT_INDEX(uint64_t virt) { return (virt >> PDPT_SHIFT) & 0x1FF; }
uint64_t PD_INDEX(uint64_t virt)   { return (virt >> PD_SHIFT) & 0x1FF; }
uint64_t PT_INDEX(uint64_t virt)   { return (virt >> PT_SHIFT) & 0x1FF; }
...
uint64_t sim_phys_of(void* ptr) {
    auto offset = static_cast<uint8_t*>(ptr) - &sim_memory[0][0];
    return MOCK_POOL_BASE + static_cast<uint64_t>(offset);
}
